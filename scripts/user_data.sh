...
APP_REPO_TOKEN="__APP_REPO_TOKEN__"
...
echo "[1/5] Installing system dependencies... $(date -u)"
apt-get update -q
apt-get install -y git python3-pip libgl1 libglib2.0-0
echo "[1/5] DONE $(date -u)"

echo "[2/5] Cloning repo... $(date -u)"
cd /home/ubuntu
git clone "$REPO_URL" training || { echo "ERROR: git clone failed"; exit 1; }
cd training
git checkout "$GIT_SHA" || { echo "ERROR: git checkout failed"; exit 1; }
echo "[2/5] DONE $(date -u)"

echo "[3/5] Installing Python dependencies... $(date -u)"
pip3 install ultralytics mlflow boto3 Pillow whylogs --quiet || { echo "ERROR: pip install failed"; exit 1; }
...
echo "[3/5] DONE $(date -u)"

echo "[4/5] Starting training... $(date -u)"
...
echo "[4/5] DONE $(date -u)"

echo "[5/5] Uploading training log to S3... $(date -u)"
aws s3 cp "$LOG_FILE" \
  "s3://$MODELS_BUCKET/crack-seg/$GIT_SHA/training.log" || echo "WARNING: log upload failed"
echo "[5/5] DONE $(date -u)"
...
sed -i "s|__APP_REPO_URL__|${app_repo_url}|g"           /home/ubuntu/run_training.sh
sed -i "s|__APP_REPO_TOKEN__|${app_repo_token}|g"       /home/ubuntu/run_training.sh
